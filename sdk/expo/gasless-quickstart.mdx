...
<Tabs>
  <Tab title="Clerk">
  1. Create a new [JWT Template](https://dashboard.clerk.com/last-active?path=jwt-templates)

  2. Copy your [Jwks Endpoint](https://clerk.com/docs/backend-requests/jwt-templates) and paste it into the JWKS Endpoint field in your Chipi Dashboard.
  </Tab>

  <Tab title="Firebase">
  We have an active PR for Firebase and will be available by next week (27nd of August 2025)
   so stay tuned!
...
```bash npm
npm install @chipi-pay/chipi-expo@latest
```

```bash yarn
yarn add @chipi-pay/chipi-expo@latest
```

```bash pnpm
pnpm add @chipi-pay/chipi-expo@latest
```
